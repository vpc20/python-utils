...
def calculate_field_size(pic_clause: str, comp_type: str = None) -> int:
    ...
    Args:
        ...
        comp_type: Computational type (COMP, COMP-3, etc.)
    ...
    if comp_type:
        ...
        if 'COMP-3' in comp_type or 'PACKED-DECIMAL' in comp_type:
            # COMP-3: packed decimal - (digits + 1) / 2
            return (total_digits + 1) // 2
        elif comp_type == 'COMP' or 'BINARY' in comp_type:
...
def parse_cobol_record(file_content: str) -> List[str]:
    ...
    Returns:
        List of SUBSTR function strings
    """
    ...
    for line in lines:
        ...
        # Find COMP type
        for part in parts:
            if 'COMP' in part.upper() or 'BINARY' in part.upper():
                ...
                for child in children:
                    if child['pic']:  # Only elementary items
                        ...
                        for child_occur in range(child['occurs']):
                            substr_list.append(f"SUBSTR({record_name}, {position}, {size})")
                            position += size
...
def generate_substr_from_file(filename: str) -> str:
    ...
    Returns:
        Comma-separated SUBSTR functions
    """
